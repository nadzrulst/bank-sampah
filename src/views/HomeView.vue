    ...
    <header class="bg-white px-6 pt-8 pb-6 border-b border-gray-100">
      <div class="flex items-center justify-between">
        ...
        >
          ...
          >
            <path
              d="M10 5a2 2 0 0 1 4 0 4 4 0 0 0 3.9 4h.1v1.6c0 .8.2 1.5.6 2.2l1.2 1.7a1 1 0 0 1-.8 1.5H7.9a1 1 0 0 1-.8-1.5L8.3 9.4c.3-.7.5-1.4.5-2.2V9A4 4 0 0 0 10 5Z"
            />
    ...
    <main class="px-6 py-6 pb-28">
      ...
      >
        <div class="relative z-10">
          ...
          <div
            class="inline-flex items-center gap-2 bg-white/10 rounded-2xl px-4 py-3 mt-6 backdrop-blur"
          >
            <svg
              viewBox="0 0 24 24"
              fill="none"
            ...
            <p class="font-semibold">
              Setara Rp {{ formatCurrency(dashboardData.total_poin || 0) }}
            </p>
          </div>
        </div>
        ...
        <div class="absolute right-0 bottom-0 leading-none">
          <svg
            ...
            stroke-linecap="round"
            stroke-linejoin="round"
            class="h-[150px] w-[150px] opacity-10"
          >
      ...
      >
        <div class="flex items-start justify-between">
          <div>
            <p class="text-sm uppercase tracking-wide text-gray-500 mb-3">
              Nilai Saldo
            </p>
            <p class="text-3xl font-bold text-slate-900">
              Rp {{ formatCurrency(dashboardData.saldo_kas || 0) }}
            </p>
          </div>
          ...
          >
            ...
            >
              <path
                d="M4 8h13a2 2 0 0 1 2 2v7a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2z"
              />
      ...
      <section class="grid grid-cols-2 gap-4 mb-5">
        ...
        >
          <div
            class="w-12 h-12 rounded-2xl bg-green-50 flex items-center justify-center mb-5"
          >
            <svg
              viewBox="0 0 24 24"
              fill="none"
          ...
          <p
            class="text-sm uppercase tracking-wide text-gray-600 leading-relaxed"
          >
          ...
          <div class="flex items-end justify-between mt-6">
            <div>
              <p class="text-3xl font-bold">
                {{ dashboardData.total_setoran_kg || 0 }}
              </p>
      ...
      <section class="mb-6 grid grid-cols-2 gap-3">
        ...
        >
          ...
          >
            ...
            >
              <path
                d="M5 17h14M5 17a2 2 0 0 1-2-2V8a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2v7a2 2 0 0 1-2 2M5 17v2a2 2 0 0 0 2 2h10a2 2 0 0 0 2-2v-2M7 21v-2m10 2v-2"
              />
            </svg>
          </div>
          <p class="text-[15px] font-medium leading-tight">Jual ke Pengepul</p>
        </router-link>
        ...
        >
          ...
          <p class="text-[15px] font-medium leading-tight">Catat Biaya</p>
        </router-link>
        ...
        >
          ...
          <p class="flex-1 text-[16px] font-medium">Tukar Poin</p>
          <svg
            viewBox="0 0 24 24"
            fill="none"
            stroke="#147052"
      ...
      <section class="mb-6">
        <div class="relative">
          <svg
            ...
            stroke-linecap="round"
            stroke-linejoin="round"
            class="absolute left-5 top-1/2 -translate-y-1/2 h-7 w-7"
          >
    ...
    >
      ...
      >
        ...
        <div class="mb-5 flex items-center justify-between">
          <h3 class="text-[19px] font-semibold">Form Terima Sampah</h3>
          <button
            type="button"
        ...
        <!-- Nama Warga -->
        <div class="relative">
          <label class="text-[14px] text-[#687481]"> Nama Warga </label>
          <input
            v-model="namaWarga"
          ...
          >
            ...
            >
              <div>
                ...
                <p class="text-xs text-[#687481]" v-if="w.username">
                  @{{ w.username }}
                </p>
              </div>
              <span
                class="text-xs font-semibold text-[#147052]"
                v-if="w.no_hp"
                >{{ w.no_hp }}</span
              >
            </button>
        ...
        <div class="mt-5">
          <div class="flex items-center justify-between">
            <p class="text-[15px] font-medium">Item Sampah</p>
            <button
              type="button"
...
const filteredWargaSuggestions = computed(() => {
  ...
  return wargaList.value.filter((w) => {
    const nameMatch = String(w.nama || "")
      .toLowerCase()
      .includes(queryText);
    const userMatch = String(w.username || "")
      .toLowerCase()
      .includes(queryText);
...
const filterChips = ["Semua", "Setoran", "Penarikan", "Pembelian", "Biaya"];
...
const filteredTransactions = computed(() => {
  ...
  return transactions.value.filter((t) => {
    ...
    if (
      ...
      !activeChips.value.some((chip) => transactionTypes.includes(chip))
    ) {
      return false;
    }
...
async function loadInitialData() {
  ...
  try {
    ...
    if (
      ...
      resMaster.value?.success &&
      Array.isArray(resMaster.value.data)
    ) {
    ...
    if (
      resWarga.status === "fulfilled" &&
      resWarga.value?.success &&
      Array.isArray(resWarga.value.data)
    ) {
      wargaList.value = resWarga.value.data
        .map((w) => ({
          id_warga: w.id_warga || "",
    ...
    if (resTransaksi.status === "fulfilled") {
      if (
        resTransaksi.value?.success &&
        Array.isArray(resTransaksi.value.data)
      ) {
    ...
    if (
      resDashboard.status === "fulfilled" &&
      resDashboard.value?.success &&
      resDashboard.value.data
    ) {
...
async function submitTransaction() {
  ...
  const invalidItem = items.value.find(
    (it) => !it.kode || !it.berat || Number(it.berat) <= 0,
  );
  if (invalidItem) {
    showModal({
      title: "Perhatian",
      message: "Harap pilih kategori sampah dan isi berat lebih dari 0 kg.",
    });
    return;
  }
  ...
  const matchedWarga =
    selectedWargaObj.value ||
    wargaList.value.find(
      (w) =>
        String(w.nama || "")
          .toLowerCase()
          .trim() === nameTrimmed.toLowerCase() ||
        String(w.username || "")
          .toLowerCase()
          .trim() === nameTrimmed.toLowerCase(),
    );

  const targetUsername = matchedWarga
    ? matchedWarga.username
    : nameTrimmed.toLowerCase().replace(/[^a-z0-9]/g, "");
  ...
  try {
    ...
    if (res && res.success) {
      ...
      await showModal({
        title: "Gagal",
        message: "Gagal menyimpan transaksi: " + errMsg,
      });
    }
  } catch (e) {
    ...
    await showModal({
      title: "Gagal",
      message: "Gagal mengirim data setoran: " + e.message,
    });
  } finally {
